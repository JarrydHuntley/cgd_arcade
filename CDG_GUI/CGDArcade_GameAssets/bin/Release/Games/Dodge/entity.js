...
(function() {
  ...
  window.Player = (function(_super) {
    ...
    Player.prototype.update = function() {
      var input;
      input = g.input;
      this.acceleration += g.gravity;
      this.checkBeat();
      return this.y += this.acceleration;
    };

    Player.prototype.checkBeat = function() {
      var boost, gap;
      if (this.lastBeat + this.gap <= g.now) {
        this.lastBeat = null;
        this.firstBeat = false;
      }
      if (this.parent.inputBool) {
        if (!this.firstBeat) {
          this.firstBeat = true;
          return this.lastBeat = g.now;
        } else {
          gap = g.now - this.lastBeat;
          boost = this.boost + (this.boost * (1 - (gap / this.gap)));
          console.log(boost);
          this.acceleration += boost;
          return this.firstBeat = false;
        }
      }
    };
